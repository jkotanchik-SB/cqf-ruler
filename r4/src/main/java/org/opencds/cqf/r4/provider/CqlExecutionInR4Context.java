...
public class CqlExecutionInR4Context extends CqlExecutionInContext<DomainResource> {

    private JpaDataProvider provider;

    public CqlExecutionInR4Context(BaseFhirDataProvider dataProvider, BaseLibraryLoader libraryLoader) {
    ...
    public Iterable<IBaseReference> getLibraryReferences(DomainResource instance) {
        ...
        if (instance instanceof ActivityDefinition) {
            references.addAll(
                    ((ActivityDefinition) instance).getLibrary().stream()
                            .map(x -> new Reference(x.getValue())).collect(Collectors.toList())
            );
        }

        else if (instance instanceof PlanDefinition) {
            references.addAll(
                    ((PlanDefinition) instance).getLibrary().stream()
                            .map(x -> new Reference(x.getValue())).collect(Collectors.toList())
            );
        }
            ...
            references.addAll(
                    ((Measure) instance).getLibrary().stream()
                            .map(x -> new Reference(x.getValue())).collect(Collectors.toList())
            );
        }
