...
public class JpaDataProvider extends FhirDataProviderR4 {
    ...
    public synchronized List<IResourceProvider> getProviders() {
        return this.resourceProviders;
    }

    public JpaDataProvider(List<Object> providers) {
        resourceProviders = new ArrayList<>();
        for (Object provider : providers) {
            resourceProviders.add((IResourceProvider) provider);
        }
                                                  ...
                                                  String dateLowPath, String dateHighPath, Interval dateRange)
    {
        ...
        if (context != null && context.equals("Patient") && contextValue != null) {
            if (dataType.equals("Patient")) {
                ...
                map.add("_id", patientParam);
            }
            else if (isPatientCompartment(dataType))
            {
                ReferenceParam patientParam = new ReferenceParam(contextValue.toString());
