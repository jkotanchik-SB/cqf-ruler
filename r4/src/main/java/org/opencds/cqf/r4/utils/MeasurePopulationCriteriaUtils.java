...
import org.opencds.cqf.r4.operation.measure.MeasurePopulationType;
import org.opencds.cqf.r4.operation.measure.MeasureScoring;
                ...
                if (populationType != null) {
                    switch (populationType) {
                        case INITIALPOPULATION:
                            ...
                            if (report.getType() == MeasureReport.MeasureReportType.SUBJECTLIST) {
            ...
            switch (measureScoring) {
                ...
                case RATIO: {
                    ...
                    if (denominator != null && numerator != null && denominator.size() > 0) {
                        reportGroup.setMeasureScore(new Quantity().setValue(numerator.size() / (double)denominator.size()));
                    }
    ...
    private static void addPopulationCriteriaReport(MeasureReport report, MeasureReport.MeasureReportGroupComponent reportGroup, Measure.MeasureGroupPopulationComponent populationCriteria, int populationCount, Iterable<Patient> patientPopulation) {
        if (populationCriteria != null) {
            ...
            if (report.getType() == MeasureReport.MeasureReportType.SUBJECTLIST && patientPopulation != null) {
                ...
                populationReport.setSubjectResults(new Reference().setReference("#" + patientList.getId()));
